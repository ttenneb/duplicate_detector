...
def getfile(args):
    ...
    print(data.shape)
    data = data[(data["Validation"] == True) | (data["Explanation"] == 'Duplicate')]
    print(data.shape)
    return data
...
def add_image_features(dataset):
    t = Timer()
    t.start()
    size = 100
    accuracy_total = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path', type=str, default="", help='csv path')

    args = parser.parse_args()

    trespassing_data = getfile(args)
    ramsey_gen = create_dataset(trespassing_data[:20])
    ramsey_gen = add_image_features(ramsey_gen)
    ...
    with open('data.csv', 'w', newline='') as csvfile:
        ...
        output.writerow(
            ['video1', 'video2', "trajectory_distance", "ln_trajectory_distance", "start_distance", "ln_start_distance",
             'seconds', 'coefficient1', 'intercept1', 'coefficient2', 'intercept2', 'is_last_duplicate',
             'duplicate_count',
             'duplicate', 'ssim'])
        for row in ramsey_gen:
            output.writerow(row)
